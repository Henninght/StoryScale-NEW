...
export function Step4SummaryAction({ onGenerate }: Step4Props) {
  ...
  // Lead magnet fields (only for lead generation goal)
  ...
  const isLeadGeneration = data.step1.goal === 'generate-leads'
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Lead Magnet Options (only for lead generation) */}
      {isLeadGeneration && (
        <div className="border rounded-lg p-4 bg-yellow-50 border-yellow-200">
          <div className="mb-4">
            <h4 className="text-sm font-medium text-gray-900 flex items-center space-x-2">
              <span>🎯</span>
              <span>Lead Generation Options</span>
            </h4>
            <p className="text-xs text-gray-600 mt-1">Customize your lead capture approach</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              >
                <option value="">Choose lead magnet type...</option>
            ...
            {leadMagnetType && (
              <div>
                ...
                <input
                  ...
                  placeholder={`e.g., The Ultimate ${leadMagnetType === 'guide' ? 'Guide to' : leadMagnetType === 'template' ? 'Template for' : leadMagnetType === 'checklist' ? 'Checklist for' : 'Tool for'} [Your Topic]`}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
            )}
            ...
            <div className="flex items-center justify-between">
              <div>
                <label className="text-sm font-medium text-gray-700">Add Scarcity Element</label>
                <p className="text-xs text-gray-600">Limited time or spots available</p>
              </div>
              <button
                onClick={() => setScarcityEnabled(!scarcityEnabled)}
                className={cn(
                  "relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2",
                  scarcityEnabled ? "bg-blue-600" : "bg-gray-200"
                )}
                role="switch"
                aria-checked={scarcityEnabled}
              >
                <span
                  className={cn(
                    "inline-block h-4 w-4 transform rounded-full bg-white transition-transform",
                    scarcityEnabled ? "translate-x-6" : "translate-x-1"
                  )}
                />
              </button>
            </div>
            ...
            {/* Preview */}
            {(leadMagnetType || scarcityEnabled) && (
              <div className="mt-4 p-3 bg-white border border-gray-200 rounded-lg">
                <p className="text-xs font-medium text-gray-700 mb-1">CTA Preview:</p>
                <p className="text-sm text-gray-900">
                  {leadMagnetType && leadMagnetTitle 
                    ? `🎯 Comment "YES" for my free ${leadMagnetTitle}`
                    : leadMagnetType 
                    ? `🎯 Comment "YES" for my free ${leadMagnetType}`
                    : "🎯 Comment \"YES\" for more information"
                  }
                  {scarcityEnabled && " (Limited spots available!)"}
                </p>
              </div>
            )}
          </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start space-x-3">
          <svg className="w-5 h-5 text-blue-600 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm-7-4a1 1 0 011-1h4a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1v-2zm8-1a1 1 0 000-2h-3a1 1 0 000 2h3z" clipRule="evenodd" />
          </svg>
          <div>
